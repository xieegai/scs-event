...
import org.baihe.scs.event.annotation.EventHandler;
import org.baihe.scs.event.utils.EventBindingBeanDefinitionRegistryUtils;
import org.baihe.scs.event.utils.EventHandlerMethodUtils;
...
public class EventHandlerAnnotationBeanPostProcessor implements BeanPostProcessor, ApplicationContextAware,
    ...
    private final MultiValueMap<Class<?>, StreamListenerHandlerMethodMapping> mappedListenerMethods = new LinkedMultiValueMap<>();
    ...
    public final void afterSingletonsInstantiated() {
        ...
        for (Map.Entry<Class<?>, List<StreamListenerHandlerMethodMapping>> mappedBindingEntry : mappedListenerMethods
                .entrySet()) {
            ...
            applicationContext.getBean(EventBindingBeanDefinitionRegistryUtils.resolveInputBindingName(mappedBindingEntry.getKey()),
                    SubscribableChannel.class).subscribe(handler);
//            EventListenerCache.getEventInputChannelBean(mappedBindingEntry.getKey(), this.applicationContext).subscribe(handler);
        }
    ...
    public final Object postProcessAfterInitialization(Object bean, final String beanName) throws BeansException {
        ...
        for (Method method : uniqueDeclaredMethods) {
            ...
            if (eventHandler != null && !method.isBridge()) {

                Class<?> eventPayloadClass = eventHandler.value();
                EventBindingBeanDefinitionRegistryUtils.registerInputBindingTargetBeanDefinition(
                    eventPayloadClass, beanFactory, null
                );
...
//        EventListenerCache.registerListenedEvent(eventHandler.value(), method, this.applicationContext);
    ...
    private class DefaultEventHandlerSetupMethodOrchestrator implements EventHandlerSetupMethodOrchestrator {
        ...
        public void orchestrateStreamListenerSetupMethod(EventHandler eventHandler, Method method, Object bean) {
            Class<?> eventPayloadClass = eventHandler.value();
            String methodAnnotatedInboundName = EventBindingBeanDefinitionRegistryUtils.resolveInputBindingName(eventPayloadClass);
        ...
        private void registerHandlerMethodOnListenedChannel(Method method, EventHandler eventHandler, Object bean) {
            ...
            mappedListenerMethods.add(eventHandler.value(),
                    new StreamListenerHandlerMethodMapping(bean, method, eventHandler.condition(), defaultOutputChannel,
